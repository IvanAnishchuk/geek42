...
|--------|---------|--------------|
| Export requirements | `uv run python scripts/regen_requirements.py --stdout` | Exports prod requirements from `uv.lock` to stdout on demand (also `--output-dir DIR` writes both files). Never committed — `uv.lock` is the source of truth. |
| Supply-chain audit | `uv run python scripts/audit.py` | Runs the full audit locally in the same order as CI: uv lock check, then `pip-audit` on prod and prod+dev (exported to a tempdir) plus a CycloneDX SBOM for prod. |
...
`uv.lock` is the single source of truth and **must be committed** with
every dependency change. `requirements*.txt` are never committed — they
are exported from `uv.lock` on demand (by `scripts/audit.py` for the
audit, or `scripts/regen_requirements.py` for a build/release artifact).

### Why uv.lock is the single source of truth

`requirements*.txt` are **not** committed. `uv.lock` is the canonical
record, and everything that needs a `requirements.txt` exports one on
demand:

- `uv.lock` — uv reads it for `uv sync`; it pins the full graph
- `scripts/audit.py` exports prod and prod+dev sets into a tempdir for
  `pip-audit` and `cyclonedx-py`, so nothing stale lands in the repo
- `scripts/regen_requirements.py --stdout` / `--output-dir` produces the
  same files for a build dir or a release SBOM (git-ignored locations only)

Committed `requirements*.txt` used to be regenerated by Dependabot and
pushed back with `GITHUB_TOKEN`, which never re-triggers CI — so those
security-relevant bumps could land unaudited. Dropping the committed
files closes that gap; security scanners read `uv.lock` directly.
...
Because `requirements*.txt` are no longer committed, those PRs only
touch `pyproject.toml` / `uv.lock` — the audit job exports and scans
them in CI, so there's nothing to regenerate on the branch.
...
This runs all four checks (uv lock consistency, pip-audit on prod,
pip-audit on prod+dev, SBOM prod) and writes reports to
`.reports/audit/`. The exact same logic runs
